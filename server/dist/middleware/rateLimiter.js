...
class RateLimiter {
    ...
    createRateLimit(options = {}) {
        ...
        return (req, res, next) => {
            ...
            res.setHeader("X-RateLimit-Remaining", Math.max(0, config.maxRequests - data.count));
            res.setHeader("X-RateLimit-Reset", Math.ceil(data.resetTime / 1000));
            // Check if limit exceeded
            if (data.count > config.maxRequests) {
                res.status(429).json((0, utils_1.errorMessage)(config.message));
                return;
            }
            next();
        };
